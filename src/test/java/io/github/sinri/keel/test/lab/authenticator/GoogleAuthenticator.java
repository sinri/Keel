...
//            var decoded=Base64.decodeBase64(SEED);
            var decoded = Base64.getDecoder().decode(SEED);
            sr.setSeed(decoded);
...
//            byte[] bEncodedKey = codec.encode(buffer);
            ...
            byte[] bEncodedKey = Keel.helpers().binary().encodeWithBase32(buffer);
    ...
    public static void main(String[] args) {
        ...
        System.out.println("generateSecretKey: " + generateSecretKey);

        boolean correct = new GoogleAuthenticator()
                .check_code(
                        "67YJGPVA63LAHZKYPJKT",
                        653449,
                        System.currentTimeMillis()
                );
        System.out.println("corrent: " + correct);
    }
...
//        byte[] decodedKey = codec.decode(secret);
        ...
        byte[] decodedKey = Keel.helpers().string().decodeWithBase32ToBytes(secret);
