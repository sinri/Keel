...
public class FutureForEachParallel {
    ...
    public static void main(String[] args) {
        ...
        FutureForEachParallel.call(List.of(1, 2, 3, 4), x -> {
                    System.out.println("GO " + x);
                    if (x % 2 == 0) {
                        return Future.failedFuture(new Exception("even: " + x));
                    } else {
                        return Future.succeededFuture();
                    }
                })
                .onComplete(ar -> {
                    if (ar.succeeded()) {
                        ...
                        ParallelResult parallelResult = ar.result();
                        parallelResult.results().forEach(parallelResultPart -> {
                            System.out.println("> " + (parallelResultPart.isFailed() ? parallelResultPart.getCause() : parallelResultPart.getResult()));
                        });
                    } else {
